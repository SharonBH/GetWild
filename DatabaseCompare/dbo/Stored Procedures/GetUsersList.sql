...
WHERE u.StudioId = @StudioId and --isnull(us.Frozen,0) = case when @Frozen = 1 then 1 else isnull(us.Frozen,0) end
...
and us.Active = ISNULL(@active,us.Active) --and u.Marked = case when @RemoveMarked = 1 then 0 else u.Marked end
...
LEFT JOIN dbo.SubscriptionType st ON st.Id = us.SubscriptionTypeId
WHERE u.StudioId = @StudioId --and --isnull(us.Frozen,0) = case when @Frozen = 1 then 1 else isnull(us.Frozen,0) end
and (u.FullName like '%' + @Search + '%' or u.UserName like '%' + @Search + '%' or u.PhoneNumber like '%' + @Search + '%')
