...
-- =============================================
CREATE PROCEDURE [dbo].[GetUsersList1]
	@DayNo INT = -1, 
	@StartDate DATE = NULL,
	@CompanyId INT = 0,
	@PageNo int = 1,
...
BEGIN
 ...
 --   -- Insert statements for procedure here
	...
	;WITH LastClass AS
(
   SELECT ct.Name, c.Date, ce.SubscriptionId,
         ROW_NUMBER() OVER (PARTITION BY ce.SubscriptionId ORDER BY c.Date DESC) AS rn
    from ClassEnrollment ce
join class c on c.id = ce.classid
join ClassType ct on ct.Id = c.ClassTypeId
where ce.IsDeleted = 0 and c.IsDeleted = 0 and c.Date < GETUTCDATE()
)

,NextClass AS
(
   SELECT ct.Name, c.Date, ce.SubscriptionId,
         ROW_NUMBER() OVER (PARTITION BY ce.SubscriptionId ORDER BY c.Date) AS rn
    from ClassEnrollment ce
join class c on c.id = ce.classid
join ClassType ct on ct.Id = c.ClassTypeId
where ce.IsDeleted = 0 and c.IsDeleted = 0 and c.Date > GETUTCDATE()
...
, Classs AS 
(
select * from LastClass where rn = 1
)
	...
	SELECT  u.FirstName,
							...
							--(select date from LastClass where rn = 1 and SubscriptionId = us.id) LastClassDate,
							--(select Name from LastClass where rn = 1 and SubscriptionId = us.id) LastClassType,
							--LastClass.Date LastClassDate, 
							--LastClass.Name LastClassType,
							--NextClass.Date NextClassDate,
							--NextClass.Name NextClassType,
...
left join Classs on Classs.SubscriptionId = us.Id
--left join LastClass on LastClass.SubscriptionId = us.Id and LastClass.rn = 1
--left join (select * from LastClass where rn = 1) lc on lc.SubscriptionId = us.Id
--left join NextClass on NextClass.SubscriptionId = us.Id and NextClass.rn = 1
WHERE s.CompanyId = @CompanyId and --isnull(us.Frozen,0) = case when @Frozen = 1 then 1 else isnull(us.Frozen,0) end
--LastClass.rn = 1 and
isnull(us.Frozen,0) = case when @Frozen = 1 then 1 else isnull(us.Frozen,0) end
